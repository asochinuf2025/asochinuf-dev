...
import { TrendingUp, Users, Database, Calendar, Activity } from 'lucide-react';
        ...
        {stats?.distribucionPosicion && stats.distribucionPosicion.filter(item => item.posicion !== 'Sin especificar').length > 0 ? (
          ...
          >
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={stats.distribucionPosicion
                    .filter(item => item.posicion !== 'Sin especificar')
                    .map((item, index) => ({
                      name: item.posicion,
          ...
          <motion.div
            ...
            className={`${
              isDarkMode ? 'bg-[#1a1c22] border-[#8c5cff]/20' : 'bg-white border-purple-200'
            } border rounded-2xl p-6 text-center flex items-center justify-center`}
        ...
        {stats?.distribucionPorZona && stats.distribucionPorZona.length > 0 ? (
          ...
          >
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={stats.distribucionPorZona.map((item) => ({
                    name: item.zona,
                    value: parseInt(item.cantidad) || 0,
                    color: ZONE_COLORS[item.zona]
                  }))}
                  cx="50%"
                  cy="50%"
                  labelLine={false}
